...
async function main() {
  ...
  console.log(`Grounding: perplexity ${perplexityModel}  (${groundingPath})`)
  console.log(`Writing:   anthropic  ${anthropicModel}  (${writingPath})`)
  ...
  if (!skipGrounding) {
    ...
    const bySport = groupBySport(needsGrounding)
    const sports = [...bySport.keys()].sort()
    for (const sport of sports) {
      const list = bySport.get(sport)!
      console.log(`\n${sport}: ${list.length} session(s)`)
      for (let i = 0; i < list.length; i++) {
        const session = list[i]
        console.log(`  [${i + 1}/${list.length}] ${session.id}`)
        if (dryRun) continue
        const g = await fetchGrounding(perplexityKey!, session, sport, perplexityModel)
        if (g) {
          ...
          console.log(`    facts: ${g.groundingFacts.length}, news: ${g.relatedNews.length}`)
        }
  ...
  if (!skipWriting && anthropicClient) {
    ...
    for (const sport of sports) {
      const list = bySport.get(sport)!
      const batches = chunk(list, WRITING_BATCH_SIZE)
      console.log(`\n${sport}: ${list.length} session(s) in ${batches.length} batch(es)`)
      const groundingForSport = new Map<string, GroundingData>()
      for (const s of list) {
        const g = groundingCheckpoint.results[s.id]
        if (g) groundingForSport.set(s.id, g)
      }
      for (let i = 0; i < batches.length; i++) {
        const batch = batches[i]
        console.log(`  Batch ${i + 1}/${batches.length} (${batch.length} sessions)`)
        if (dryRun) continue
        const results = await generateWriting(anthropicClient, batch, sport, groundingForSport, anthropicModel)
        const gotIds = new Set<string>()
        for (const r of results) {
          if (!sessionMap.has(r.id)) continue
          writingCheckpoint.results[r.id] = r
          gotIds.add(r.id)
          writingResults.push(r)
        }
        writingCheckpoint.updatedAt = new Date().toISOString()
        writeJson(writingPath, writingCheckpoint)
        const missing = batch.filter((s) => !gotIds.has(s.id))
        if (missing.length > 0) {
          console.warn(`    missing: ${missing.map((s) => s.id).join(', ')}`)
        }
        console.log(`    checkpointed ${Object.keys(writingCheckpoint.results).length}`)
